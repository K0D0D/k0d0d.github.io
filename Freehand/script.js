...
function openMenu(){
    ...
    opener.addEventListener("click", () => {//открытие меню
        //далее последуют танцы с бубном дабы вычислить ширину скролбара
...
function swipeSlider(){
    ...
    function swipeEnd(){//окончание свайпа
        oldIndex = undefined;//необходимо для одноразовой проверки с currIndex
        ...
        slides.forEach((item) => {
            item.style.transition = "0.3s";//плавность перемещения слайдов
            if(dist >= slideWidth / 6 && currIndex > 0){//если пользователь пролистал вправо достаточно, и текущий индекс не 0
                if(currIndex != oldIndex){//уменьшить индекс только раз, поскольку это цикл, нам не нужно чтобы это происходило многократно
                    currIndex--;
                    oldIndex = currIndex;
                }
                item.style.transform = "translateX(-" + currIndex * slideWidth + "px)";//перенос слайдов
            }
            else if(-dist >= slideWidth / 6 && currIndex < slides.length - 1){//если пользователь пролистал влево достаточно 
            ...
            else{//если всё таки пролистали недостаточно, то слайды возвращаются на исходную позицию
                item.style.transform = "translateX(" + translateVal + "px)";
            }
            translateVal = Number(item.style.transform.match(/(-[0-9]|[0-9])+/ig));//получение численного значения transform: translateX
        });
